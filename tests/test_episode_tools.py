...
async def _episode_id(settings: Settings, fault: str, *, open_only: bool = False) -> int:
    result = await episodes.list_episodes(settings=settings, fault=fault, days=365)
    rows = [r for r in result["episodes"] if not open_only or r["ended_at"] is None]
    return int(rows[0]["episode_id"])


async def test_list_episodes_resolves_subject_against_the_catalog(
    settings: Settings, clean_verdicts: None
) -> None:
    result = await episodes.list_episodes(settings=settings, days=365)
...
async def test_list_episodes_filters_by_state_and_window(
    settings: Settings, clean_verdicts: None
) -> None:
    open_only = await episodes.list_episodes(settings=settings, state="open", days=365)
    assert [r["fault"] for r in open_only["episodes"]] == ["silence"]
    assert all(r["ended_at"] is None for r in open_only["episodes"])

    ended_only = await episodes.list_episodes(settings=settings, state="ended", days=365)
    ...
    recent = await episodes.list_episodes(settings=settings, days=7)
    assert {r["fault"] for r in recent["episodes"]} == {"silence"}
    assert recent["row_count"] == 2


async def test_set_verdict_is_read_back_on_the_episode(
    settings: Settings, clean_verdicts: None
) -> None:
    episode_id = await _episode_id(settings, "silence", open_only=True)

    written = await episodes.set_episode_verdict(
        settings=settings, episode_id=episode_id, verdict="nonsense"
    )
    assert written["verdict"] == "nonsense"
    assert written["episode_id"] == episode_id
    assert written["fault"] == "silence"

    listed = await episodes.list_episodes(settings=settings, days=365)
...
async def test_second_verdict_overwrites_and_never_duplicates(
    ...
    await episodes.set_episode_verdict(settings=settings, episode_id=episode_id, verdict="nonsense")
    second = await episodes.set_episode_verdict(
        settings=settings, episode_id=episode_id, verdict="real"
    )
...
async def test_unknown_verdict_names_the_valid_ones(
    ...
    with pytest.raises(ValueError, match="nonsense"):
        await episodes.set_episode_verdict(
            settings=settings, episode_id=episode_id, verdict="maybe"
        )


async def test_verdict_on_an_unknown_episode_is_a_precise_error(
    settings: Settings, clean_verdicts: None
) -> None:
    with pytest.raises(ValueError, match="999999"):
        await episodes.set_episode_verdict(settings=settings, episode_id=999999, verdict="real")
...
async def test_read_only_server_serves_reads_and_refuses_verdicts(settings: Settings) -> None:
    ...
    try:
        listed = await episodes.list_episodes(settings=read_only, days=365)
        assert listed["row_count"] > 0
        with pytest.raises(RuntimeError, match="MCP_DB_WRITE_USERNAME"):
            await episodes.set_episode_verdict(
                settings=read_only, episode_id=listed["episodes"][0]["episode_id"], verdict="real"
            )
    finally:
        await db.close_pool()
